	...
	fetch('music_data.csv')
		...
		.then(data => {
			...
			for (let i = 1; i < lines.length; i++) {
				...
				// 정규식을 사용하여 CSV 파싱 (쉼표가 포함된 필드 처리)
				const values = parseCSVLine(lines[i]);
				if (values.length >= 6) {
					const row = {
						order: i,
						artist: values[0].trim(), // singer
						song: values[1].trim(),   // title
						cheeze: values[2].trim(), // cheeze
						genre: values[3].trim(),  // genre
						cover_link: values[4].trim(), // cover_image
						category: values[5].trim()   // country (K-POP, J-POP)
					};
					musicbook.push(row);
				}
			}
...
function random_select(jsonObj, num) {
	...
	for (i; i < num; i = i + 1) {
		...
		// cheeze 값이 있으면 정보 표시
		if (musiclist[rnd].cheeze && musiclist[rnd].cheeze.trim() !== '') {
			var cheezeInfo = document.createElement('formatted-string');
			cheezeInfo.classList.add("random-cheeze-info");
			cheezeInfo.textContent = musiclist[rnd].cheeze;
			infoDiv.appendChild(cheezeInfo);
		}
...
function populateSection(jsonObj, direction) {
	...
	for (i; i != end; i = i + direction) {
		...
		// cheeze 값이 있으면 정보 표시
		if (musiclist[i].cheeze && musiclist[i].cheeze.trim() !== '') {
			var cheezeInfo = document.createElement('formatted-string');
			cheezeInfo.classList.add("cheeze-info");
			cheezeInfo.textContent = musiclist[i].cheeze;
			infoDiv.appendChild(cheezeInfo);
		}
